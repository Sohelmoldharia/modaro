...
function madara_wp_manga_chapters_sort( $sort_option ){
	$sort_option = App\MangazScans::getOption('manga_chapters_order', 'name_desc');
	...
	$reading_manga_chapters_order = App\MangazScans::getOption('manga_chapters_select_order', 'default');
    ...
    $sort_option['vol_order'] = App\MangazScans::getOption('manga_volumes_order', 'desc');
...
function madara_change_latest_chaptes_count( $count ){
    $count = MangazScans::getOption('manga_archive_latest_chapters_visible', $count);
...
function madara_wp_manga_meta_visibility( $visibility ){
    $count = MangazScans::getOption('manga_archive_latest_chapters_visible', 1);
...
function madara_speaker_body_classes( $classes ){
    $speaker_sized = MangazScans::getOption('speaker_sized', '');
    if($speaker_sized){
        $classes[] = 'speaker-sized';
    }
    
    $speaker_position = MangazScans::getOption('speaker_position', '');
    if($speaker_position){
