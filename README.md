...
1. Build the install zip:
   ...
   ./build-zip.sh
   ```
   Produces `dist/mangazscans.zip`.
2. WP Admin → **Appearance → Themes → Add New → Upload Theme** → pick `mangazscans.zip`.
3. Activate. WP will prompt you to install the bundled **Madara - Core** and **Option Tree** plugins via TGM. Install both.
4. Go to **Appearance → Theme Options** to configure colors, layout, manga reading style, etc.
5. (Optional) Customizer → **Site Identity** → upload your own logo / Site Icon. The theme ships clean defaults at:
   - `images/logo.svg` + `logo.png` (wordmark)
   - `images/favicon.svg` + `favicon.png` (browser tab icon)
   These are auto-loaded only if you don't upload your own.
